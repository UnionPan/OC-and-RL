...
class DQNAgent:
    ...
    def Network(self):
        model = Sequential()
        model.add(Dense(256, input_dim=self.d_state, activation='relu'))
...
if __name__ ==  "__main__":
    aa_agent = DQNAgent(train = TRAIN)
    aa_agent.main()
    #ep_time = np.linspace(0, len(np.array(aa_agent.ep_reward)), len(np.array(aa_agent.ep_reward)))

    #plt.plot(np.array(ep_time), np.array(aa_agent.ep_reward), 'b')
